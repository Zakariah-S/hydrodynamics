...
def sod_shock(x_start, x_end, y_start, y_end, t_final, nx, ny, nt, savename = None):
    ...
    U, t, x, y = initialize(x, y, t_final, nt, rho, vx, vy, P)
...
if __name__ == '__main__':
    sod_shock(x_start=0.,
            ...
            nx = 100,         #number of positions along x that we track
            ny = 100,         #number of positions along y that we track
            nt = 40,          #number of time steps we take over the interval t_final - 0s
            savename='testsodshock100x100')   #name of file we save data to (will have an .npz appended to it)
    pass

    animate_from_file("testsodshock100x100.npz")