...
def initialize(x, y, t_final, steps, rho, v, p, gamma=1.4, theta=1.5, cfl=0.5):
    ...
    y_expand = np.linspace(y[0] - 2*dy, y[-1] + 2*dy, y.size + 4)
    rho_expand = np.zeros((x_expand.size, y_expand.size))
    v_expand = np.zeros((x_expand.size, y_expand.size))
    p_expand = np.zeros((x_expand.size, y_expand.size))
    rho_expand[2:-2,2:-2] = rho
    v_expand[2:-2,2:-2] = v
    p_expand[2:-2,2:-2] = p
